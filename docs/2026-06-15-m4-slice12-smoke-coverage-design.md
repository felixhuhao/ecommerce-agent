...
| `warehouse_cohort` | `what is the repeat purchase rate by customer cohort over the last 12 months?` | `data-warehouse-analyst` | `query_readonly`, `list_tables`, `get_table_schema`, `metric_catalog_search`, `explain_query` | Spring operational write tools, chart tools, `request_approval` | authoritative warehouse answer |
